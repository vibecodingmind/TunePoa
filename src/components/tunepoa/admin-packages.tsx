...
export function AdminPackages() {
 ...
 Platinum: <Zap className="h-5 w-5 text-teal-400" />,
 }
 ...
 <p className="text-slate-400 text-sm mt-1">{packages.length} packages</p>
 </div>
 ...
 <p className="text-xl font-bold text-teal-400">TZS {pkg.price.toLocaleString()}</p>
 </div>
 </div>
 ...
 <div className="flex items-center gap-2">
 <Label htmlFor={`toggle-${pkg.id}`} className="text-xs text-slate-400">{pkg.isActive ? 'Active' : 'Inactive'}</Label>
