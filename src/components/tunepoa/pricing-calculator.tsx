...
export function PricingCalculator({ mode = 'landing', onSubscribe, isAuthenticated }: PricingCalculatorProps) {
 ...
 {/* User Count Card */}
 <div className="relative rounded-2xl border border-white/[0.08]/80 p-6 sm:p-7 shadow-lg shadow-black/20 hover:shadow-xl transition-shadow duration-500">
 ...
 className="h-12 w-12 flex items-center justify-center text-slate-400 hover:text-teal-400 hover:bg-teal-500/10 transition-colors duration-200 active:scale-95"
 >
 ...
 className="w-20 h-12 text-center text-xl font-bold text-white bg-transparent border-0 focus:outline-none focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
 />
 <button
 ...
 {result && (
 <div className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-teal-500/10 border border-teal-500/10 animate-fade-in">
 <div className="h-2 w-2 rounded-full bg-teal-500/100 animate-pulse" />
 ...
 className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 ${ userCount === num ? 'bg-teal-500/100 text-white shadow-md shadow-tp-500/30' : 'bg-white/5 text-slate-400 hover:bg-teal-500/10 hover:text-teal-400' }`}
 >
 {num}
 </button>
 ))}
 ...
 <div>
 <h4 className="text-sm font-bold text-white">Subscription Duration</h4>
 ...
 className={`relative py-4 px-2 rounded-xl text-center transition-all duration-300 group ${ duration === d.value ? 'bg-gradient-to-br from-tp-500 to-ts-500 text-white shadow-lg shadow-tp-500/30 scale-[1.02]' : 'bg-white/[0.03] border-2 border-white/[0.08] text-slate-300 hover:border-tp-300 hover:bg-teal-500/10/50' }`}
 >
 {d.popular && (
 ...
 <div className={`text-xl sm:text-2xl font-extrabold ${duration === d.value ? 'text-white' : 'text-white'}`}>
 {d.label}
 </div>
 ...
 <div className="text-[9px] font-bold text-teal-400 mt-1">{d.badge}</div>
 )}
 </button>
 ))}
 </div>
 ...
 <div>
 <h4 className="text-sm font-bold text-white">Audio Recording</h4>
 ...
 {includesAudio && (
 <div className="mt-4 flex items-start gap-3 p-3 rounded-xl bg-violet-500/10 border border-violet-500/20 animate-fade-in">
 ...
 {result && (
 <Badge className="bg-teal-500/100/20 text-teal-300 border-teal-500/100/30 text-[10px] font-bold uppercase tracking-wider">
 ...
 {error && (
 <div className="p-4 rounded-xl bg-red-500/100/10 border border-red-500/20 text-red-300 text-sm flex items-center gap-2">
 ...
 <p className="text-xs font-bold uppercase tracking-[0.15em] text-slate-400 mb-2">Total Cost</p>
 <div className="text-4xl sm:text-5xl font-extrabold text-white tracking-tight">
 {formatTZS(result.total)}
 <span className="text-base font-medium text-slate-400 ml-2">TZS</span>
 </div>
 ...
 <p className="text-[10px] text-slate-400 uppercase tracking-wider font-medium">Months</p>
 </div>
 </div>
 </div>
 )}
 ...
 <div className="flex flex-col items-center gap-2 p-3 rounded-xl border border-white/[0.08]/80 text-center">
 <Shield className="h-5 w-5 text-teal-400" />
 <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-tight">Secure<br/>Payment</span>
 </div>
 <div className="flex flex-col items-center gap-2 p-3 rounded-xl border border-white/[0.08]/80 text-center">
 <Headphones className="h-5 w-5 text-blue-500" />
 <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-tight">24/7<br/>Support</span>
 </div>
 <div className="flex flex-col items-center gap-2 p-3 rounded-xl border border-white/[0.08]/80 text-center">
 <Gift className="h-5 w-5 text-violet-500" />
 <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-tight">Cancel<br/>Anytime</span>
 </div>
 </div>
 </div>
 </div>
 ...
 ═══════════════════════════════════════════════════════ */
 <Card className="border-0 shadow-2xl shadow-black/30 overflow-hidden rounded-2xl">
 ...
 <button
 onClick={() => setUserCount(Math.max(1, userCount - 1))}
 className="h-11 w-11 flex items-center justify-center text-slate-400 hover:text-teal-400 hover:bg-teal-500/10 transition-colors active:scale-95"
 >
 <Minus className="h-4 w-4" />
 </button>
 <input
 ...
 value={userCount}
 onChange={(e) => setUserCount(Math.max(1, parseInt(e.target.value) || 1))}
 className="w-16 h-11 text-center text-lg font-bold text-white bg-transparent border-0 focus:outline-none focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
 />
 <button
 onClick={() => setUserCount(userCount + 1)}
 className="h-11 w-11 flex items-center justify-center text-slate-400 hover:text-teal-400 hover:bg-teal-500/10 transition-colors active:scale-95"
 ...
 {result && (
 <Badge className="bg-teal-500/10 text-teal-300 border-teal-500/10 hover:bg-teal-500/10">
 ...
 <div className="space-y-3">
 <div className="flex items-center gap-3">
 <div className="h-8 w-8 rounded-lg bg-teal-500/10 text-teal-300 flex items-center justify-center text-xs font-extrabold">2</div>
 ...
 key={d.value}
 onClick={() => setDuration(d.value)}
 className={`relative py-3 px-2 rounded-xl text-center transition-all duration-300 ${ duration === d.value ? 'bg-gradient-to-br from-tp-500 to-ts-500 text-white shadow-lg shadow-tp-500/25' : 'bg-white/[0.03] border-2 border-white/[0.08] text-slate-300 hover:border-tp-300' }`}
 >
 {d.popular && (
 <span className="absolute -top-2 left-1/2 -translate-x-1/2 px-1.5 py-0.5 rounded bg-amber-500/100 text-white text-[8px] font-bold uppercase">
 Hot
 </span>
 )}
 ...
 <div className="space-y-3">
 <div className="flex items-center gap-3">
 <div className="h-8 w-8 rounded-lg bg-teal-500/10 text-teal-300 flex items-center justify-center text-xs font-extrabold">3</div>
 <Label className="text-sm font-bold text-white">Audio Recording</Label>
 </div>
 <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/[0.08] hover:border-slate-300 transition-colors">
 <div className="flex items-center gap-3">
 <div className="h-9 w-9 rounded-lg bg-violet-500/15 text-violet-400 flex items-center justify-center">
 <Music className="h-4 w-4" />
 </div>
 <div>
 <p className="text-sm font-semibold text-slate-200">Include Audio Recording</p>
 <p className="text-xs text-slate-400">
 ...
 {/* Price Summary */}
 {result && (
 <div className="rounded-2xl bg-gradient-to-br from-teal-500/10 to-teal-500/5 border-2 border-teal-500/10 p-6 space-y-4">
 <div className="flex items-center gap-2">
 <CheckCircle2 className="h-5 w-5 text-teal-400" />
 <span className="text-sm font-bold text-teal-300">Your Plan Summary</span>
 </div>
 ...
 <span className="font-bold text-slate-200">{result.durationMonths} Months</span>
 </div>
 </div>

 <div className="h-px bg-teal-500/10/60" />

 <div className="space-y-2.5 text-sm">
 <div className="flex justify-between">
 <span className="text-slate-400">Subtotal:</span>
 <span className="font-bold text-slate-200">{formatTZS(result.subtotal)} TZS</span>
 </div>
 {result.includesAudio && (
 <div className="flex justify-between">
 ...
 <span className="font-bold text-slate-200">+{formatTZS(result.audioCost)} TZS</span>
 </div>
 )}
 </div>
 ...
 <div className="space-y-3">
 <div className="flex items-center gap-3">
 <div className="h-8 w-8 rounded-lg bg-teal-500/10 text-teal-300 flex items-center justify-center text-xs font-extrabold">4</div>
 <Label className="text-sm font-bold text-white">Select Service Request</Label>
 </div>
 {requests.length > 0 ? (
 ...
 className={`w-full p-3.5 rounded-xl border-2 text-left transition-all flex items-center justify-between ${ selectedRequest === r.id ? 'border-teal-500/100 bg-teal-500/10 shadow-sm shadow-tp-100' : 'border-white/[0.08] hover:border-slate-300' }`}
 >
 <div>
 <p className="text-sm font-semibold text-slate-200">{r.businessName}</p>
 ...
 <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/10 text-amber-400 text-sm flex items-center gap-2">
 <Info className="h-4 w-4 shrink-0" />
 ...
 <div className="space-y-3">
 <div className="flex items-center gap-3">
 <div className="h-8 w-8 rounded-lg bg-teal-500/10 text-teal-300 flex items-center justify-center text-xs font-extrabold">5</div>
 ...
 className="pl-4 h-12 text-base bg-white/5 border-white/[0.08] text-white"
 />
 </div>
 </div>
