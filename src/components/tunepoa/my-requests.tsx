...
function StatusMessage({ req }: { req: ServiceRequest }) {
 ...
 return (
 <div className="flex items-start gap-2.5 mt-3 p-3 rounded-xl bg-red-500/10 border border-red-500/10">
 ...
 return (
 <div className="flex items-start gap-2.5 mt-3 p-3 rounded-xl bg-teal-500/10 border border-teal-500/10">
 <div className="h-7 w-7 rounded-full bg-teal-500/10 flex items-center justify-center shrink-0 mt-0.5">
 <CheckCircle2 className="h-3.5 w-3.5 text-teal-400" />
 </div>
 <div>
 <p className="text-sm font-semibold text-teal-300">Request Approved</p>
...
export function MyRequests() {
 ...
 <p className="text-slate-400 text-sm mt-0.5">View and manage your ad requests</p>
 </div>
 </div>
 ...
 className="pl-9 h-11 bg-white/5 border-white/[0.08] text-white"
 />
 </div>
 <Select value={statusFilter} onValueChange={setStatusFilter}>
 <SelectTrigger className="w-full sm:w-52 h-11 bg-white/5 border-white/[0.08] text-white">
 ...
 <h3 className="font-semibold text-white">{req.businessName}</h3>
 <Badge variant="outline" className="text-xs text-slate-400 border-white/[0.08]">
 <Tag className="h-2.5 w-2.5 mr-1" />
 {req.adType}
 </Badge>
 <Badge
 variant="outline"
 className={`text-xs ${STATUS_COLORS[req.status] || 'bg-white/5 text-slate-300 border-white/[0.08]'}`}
 >
 ...
 className="text-teal-400 hover:text-teal-300 text-xs font-semibold ml-1 inline-flex items-center gap-0.5"
 >
 {isExpanded ? (
 <>
 ...
 <p className="text-sm text-slate-300 mt-1">{req.specialInstructions}</p>
 </div>
 )}
 ...
 className={`h-9 w-9 rounded-lg text-sm font-medium transition-colors ${ isCurrentPage ? 'bg-tp-600 text-white' : 'text-slate-300 hover:bg-white/5' }`}
 >
 {pageNum}
 </button>
 )
